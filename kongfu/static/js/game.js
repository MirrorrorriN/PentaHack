...
Sky = function(){
  ...
  for(var i=0; i<this.nClouds; i++){
    ...
    c.mesh.position.z = -500-Math.random()*500;
...
Sky.prototype.moveClouds = function(){
  ...
  this.mesh.rotation.z += game.speed*deltaTime*10;
}

var cloudTime = 0;
Sky.prototype.runClouds = function(){
  if(cloudTime == 1000) 
  {   
...
Sea = function(){
  ...
  var geom = new THREE.SphereGeometry(game.seaRadius,32,8);
...
Cloud.prototype.rotate = function(){
  ...
  for(var i=0; i<l; i++){
    ...
    m.rotation.x+= Math.random()*.005*(i+1);
...
EnnemiesHolder.prototype.spawnEnnemies = function(){
  ...
  for (var i=0; i<nEnnemies; i++){
    ...
    ennemy.mesh.position.x = -150 + Math.random()*300;
    ennemy.mesh.position.y = Math.random()*200;
    ennemy.mesh.position.z = -500 + Math.random()*100;
    ennemy.mesh.rotation.z = Math.random()*Math.PI*2;
    ennemy.mesh.rotation.y = Math.random()*Math.PI*2;
...
var ennemyTime = 0;
EnnemiesHolder.prototype.runEnnemies = function(){
  for (var i=0; i<this.ennemiesInUse.length; i++){
    var ennemy = this.ennemiesInUse[i];

    ennemy.mesh.position.z += 15;
...
CoinsHolder.prototype.addCoins = function(x,y,z,a){
  ...
  var coin;
  coin = new Coin();
  coin.angle = ins.xyangle + a;
  coin.distance = 0;
  coin.mesh.position.y = -90;
  coin.mesh.position.x = 80;
   this.mesh.add(coin.mesh);
  this.coinsInUse.push(coin);
}

CoinsHolder.prototype.shotCoin = function(){
  for (var i=0; i<this.nCoins; i++){
    var coin = this.coinsInUse[i];
    if (coin.exploding) continue;
    if (coin.angle>Math.PI*2) coin.angle -= Math.PI*2;
    //coin.mesh.position.y += Math.sin(coin.angle)*10;

    //coin.mesh.position.x += Math.cos(coin.angle)*10;
    coin.mesh.position.y -= 10;
    
    coin.mesh.position.x -= 10;
    coin.mesh.position.z -= 10;
    coin.mesh.rotation.z += Math.random()*.1;
    coin.mesh.rotation.y += Math.random()*.1;
    coin.distance += 100;

    /*for (var j=0; j<ennemiesPool.length; j++){
      var ennemy = ennemiesPool[j];
      var diffPos = ennemy.mesh.position.clone().sub(coin.mesh.position.clone());*/
      var d = 100;
...
CoinsHolder.prototype.rotateCoins = function(){
  for (var i=0; i<this.coinsInUse.length; i++){
    ...
    //var globalCoinPosition =  coin.mesh.localToWorld(new THREE.Vector3());
    var diffPos = ins.mesh.position.clone().sub(coin.mesh.position.clone());
    var d = diffPos.length();
    if (
      ((coin.mesh.position.x < ins.mesh.position.x + 70 && coin.mesh.position.x > ins.mesh.position.x - 70)
        ||(ins.mesh.position.x < coin.mesh.position.x + 70 && ins.mesh.position.x > coin.mesh.position.x - 70))
      &&((coin.mesh.position.y < ins.mesh.position.y + 70 && coin.mesh.position.y > ins.mesh.position.y - 70)
        ||(ins.mesh.position.y < coin.mesh.position.y + 70 && ins.mesh.position.y > coin.mesh.position.y - 70))
      &&((coin.mesh.position.z < ins.mesh.position.z + 10 && coin.mesh.position.z > ins.mesh.position.z - 10)
        ||(ins.mesh.position.z < coin.mesh.position.z + 10 && ins.mesh.position.z > coin.mesh.position.z - 10))
      ){
      this.coinsPool.unshift(this.coinsInUse.splice(i,1)[0]);
      this.mesh.remove(coin.mesh);
      particlesHolder.spawnParticles(coin.mesh.position.clone(), 5, 0x000000, .8);
...
function updatePlane(){
  ...
  var targetY = normalize(mousePos.y,-.75,.75,0, 200);
...
var Instrument = function(insTp){
  this.mesh = new THREE.Object3D();
  this.mesh.name = "instrument";
  
  var instrumentType = insTp;
  var instrument;
  var xyangle = 90;
  var zyangle = 0;
...
Instrument.prototype.update = function(){
  ...
  this.mesh.rotation.z = (targetY-this.mesh.position.y)*deltaTime*game.planeRotXSensivity;
  this.mesh.rotation.x = (this.mesh.position.y-targetY)*deltaTime*game.planeRotZSensivity;
...
function createInstrument(insTp){
  ins = new Instrument(insTp);
  //ins.mesh.position.set(80,-90,-120);
  scene.add(ins.mesh);
  //scene.add(ins.instrument.mesh);
}
...
function loop(){
 // console.log("aaaaa", plam)
  ...
  if (game.status=="playing"){
    ...
    game.speed *= .99;
    ins.mesh.rotation.z += (-Math.PI/2 -ins.mesh.rotation.z)*.0002*deltaTime;
    ins.mesh.rotation.x += 0.0003*deltaTime;
    game.planeFallSpeed *= 1.05;
    ins.mesh.position.y -= game.planeFallSpeed*deltaTime;

    if (ins.mesh.position.y <-200){
      showReplay();
  ...
  //ins.propeller.rotation.x +=.2 + game.planeSpeed * deltaTime*.005;
  sea.mesh.rotation.x += game.speed*deltaTime;//*game.seaRotationSpeed;
